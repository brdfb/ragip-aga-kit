...
argument-hint: "[listele | ekle <firma_adi> [tip=tedarikci|musteri|distributor|diger] | guncelle <id> <alan=deger> | sil <id> | ara <terim>]"
...
```json
{"id": "1", "ad": "ABC Dagitim A.S.", "tip": "tedarikci", "vergi_no": "1234567890", "iletisim": {"tel": "0212...", "email": "info@abc.com"}, "vade_gun": 60, "vade_farki_oran": 3.0, "risk_notu": "orta", "notlar": "Genelde zamaninda oder", "son_islem": "2026-02-18", "olusturma": "2026-02-18"}
```

Gecerli `tip` degerleri: `tedarikci`, `musteri`, `distributor`, `diger`

**Neden onemli:** Nakit akisi yonetiminde tedarikciye "gec ode, mevduata yatir" stratejisi uygulanirken, musteriye "erken tahsil et" stratejisi uygulanir. Tip olmadan vade farki hesaplamasi ve strateji onerileri yanlis yone isaret edebilir.
...
tip_label = {'tedarikci': 'TDR', 'musteri': 'MUS', 'distributor': 'DST', 'diger': 'DGR'}

# Tipe gore grupla
gruplama = {}
for f in firmalar:
    t = f.get('tip', 'diger')
    gruplama.setdefault(t, []).append(f)

print(f'=== FIRMA KARTLARI ({len(firmalar)}) ===')
for tip_key in ['tedarikci', 'musteri', 'distributor', 'diger']:
    grup = gruplama.get(tip_key, [])
    if not grup:
        continue
    print()
    print(f'--- {tip_key.upper()} ({len(grup)}) ---')
    for f in sorted(grup, key=lambda x: x.get('ad', '')):
        icon = risk_icon.get(f.get('risk_notu', 'orta'), '?')
        print(f\"{icon} [{f['id']}] {f['ad']} [{tip_label.get(f.get('tip','diger'),'?')}]\")
        if f.get('vergi_no'):
...
VALID_TIP = {'tedarikci', 'musteri', 'distributor', 'diger'}
...
for k, v in kvs.items():
    if k == 'tip':
        if v not in VALID_TIP:
            print(f'! Gecersiz tip: {v}. Gecerli: {VALID_TIP}')
            exit()
        yeni['tip'] = v
    elif k == 'tel':
        yeni['iletisim']['tel'] = v
...
print(f'+ Firma eklendi: [{yeni_id}] {yeni[\"ad\"]} ({yeni[\"tip\"]})')
if yeni['vergi_no']:
    print(f'   VKN: {yeni[\"vergi_no\"]}')
print(f'   Tip: {yeni[\"tip\"]} | Vade: {yeni[\"vade_gun\"]} gun | Oran: %{yeni[\"vade_farki_oran\"]}/ay')
"
```
...
sonuclar = []
for f in firmalar:
    aranacak = f'{f.get(\"ad\",\"\")} {f.get(\"tip\",\"\")} {f.get(\"vergi_no\",\"\")} {f.get(\"notlar\",\"\")}'.lower()
