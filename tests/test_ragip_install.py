...
class TestInstall:
    ...
    def test_manifest_file_count(self, temp_repo):
        """Manifest'te core dosyalar olmali (5 agent + 15 skill + 5 script + 1 config + 6 test)"""
        manifest = json.loads(
            (temp_repo / "config" / ".ragip_manifest.json").read_text()
        )
        count = len(manifest["files"])
        assert count == 32, f"Beklenen 32 dosya, bulunan {count}: {sorted(manifest['files'].keys())}"
